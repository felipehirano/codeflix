...
export abstract class InMemoryRepository<E> implements RepositoryInterface<E> {
  ...
  async update(entity: E): Promise<void> {
    ...
    const indexFound = this.items.findIndex(
      (i) => (i as any).id === (entity as any).id
    );
  ...
  async delete(id: string | UniqueEntityId): Promise<void> {
    const _id = `${id}`;
    await this._get(_id);
    const indexFound = this.items.findIndex((i) => (i as any).id === _id);
    this.items.splice(indexFound, 1);
  }

  protected async _get(id: string): Promise<E> {
    const item = this.items.find((i) => (i as any).id === id);
    if (!item) {
...
export abstract class InMemorySearchableRepositoryInterface<E extends Entity>
