...
describe('ValidatorRules Unit Tests', () => {
    ...
    it('should throw validation error when combine two or more validation rules', () => {
        ...
        expect(() => 
            validator.required().string().maxLength(5)).toThrow('The field is required'
        );

        validator = ValidatorRules.values(5, 'field');
        expect(() => 
            validator.required().string().maxLength(5)).toThrow('The field must be a string'
        );

        validator = ValidatorRules.values("aaaaaa", 'field');
        expect(() => 
            validator.required().string().maxLength(5)).toThrow('The field must be less or equal than 5'
        );

        validator = ValidatorRules.values(null, 'field');
        expect(() => 
            validator.required().boolean()).toThrow('The field is required'
        );

        validator = ValidatorRules.values('true', 'field');
        expect(() => 
            validator.required().boolean()).toThrow('The field must be a boolean'
        );
    });
